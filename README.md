...
### One Star - Dynamic Inventory

Написал PHP скрипт для динамической инвентаризации.

Основные файлы (классы) PHP раположил в директории `ansible/dynamic`

К сожалению не успеваю прикрутить веб интерфейс и sqlite ~как изначально сделал~.

Забил тестовыми данными, для примера, в файл `ansible/dynamic.php`

Пример использования: `cd ./ansible/ && ansible all -m ping -i dynamic.php`


## Homework 11

Playbook -и которые использует Packer, `packer_app.yml` & `packer_db.yml`, разместил в директории `packer`.

`packer_app.yml` & `packer_db.yml` немного сделал по своему, в исследовательских целях, в обязательном порядке проверил работоспособность и в конце сверился с приведенным примером gist


## Homework 12

Playbook -и которые использует Packer, `packer_app.yml` & `packer_db.yml`, перенес обратно в директорию `ansible`.

Добавил роли в ансибл

С помощью ansible настроил проксирование запросов к приложение через Nginx, по порту 80.

Но возникла сложность - nginx работает штатно, но само приложение иногда скидывает сессию авторизации.
Исправить эту ситуацию на уровне "ops" не удалось.
...
Настроил provision вагранта для работы с ansible

Добавил в вагрнат настройки проксирования nginx


**molecule**

добавил в роль `db` тест, который проверяет, слушает ли mongoDB нужный адрес и порт

**packer**

перевел `packer` на работу с ролями `ansible`
